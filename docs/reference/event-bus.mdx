...
Solidus comes with a publish-subscribe system provided by [Omnes](https://github.com/nebulab/omnes)
(a pub/sub library for Ruby). It allows applications to hook into Solidus events (like completing an
order) and extend the associated behavior.
...
options to [customize the Solidus core](/tutorials/your-first-store/customize-the-core.mdx).
...
module MyStore
  class SmsSubscriber
    ...
    handle :order_finalized, with: :notify_order_completed, id: :sms_notify_order_completed
...
There are other possible ways to create subscriptions with Omnes. You can, for instance, subscribe to
...
Once you run the [update generator](/concepts/maintenance-practices.mdx#updating-preferences),
