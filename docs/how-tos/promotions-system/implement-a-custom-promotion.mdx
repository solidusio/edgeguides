# Implement a custom promotion

You're not limited to using the stock Solidus functionality. In fact, the promotion system shows
its full potential when you use it to implement your own logic. In the rest of the guide, we'll use
the promotion system to implement the following requirements:
...
state (see
the [section on how to customize state machines](/reference/state-machines.mdx#customizing-core-behavior)):
