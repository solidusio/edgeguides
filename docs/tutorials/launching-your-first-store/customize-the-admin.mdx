...
override (see the [overrides section](/tutorials/your-first-store/customize-the-core.mdx#using-overrides) for how to set it
upeckout/address) to accomplish that:
...
codebase, we can see the view in question is `spree/adminz/orders/edit.html.erb`. If we inspect the
...
seen [how to override views with Deface](/tutorials/your-first-store/customize-the-admin.mdx#defacing-admin-views). This time
...
the [previously introduced filter](/tutorials/your-first-store/customize-the-admin.mdx#adding-new-search-form-fields) is
